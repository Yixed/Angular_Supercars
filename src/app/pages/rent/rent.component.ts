...
@Component({
  selector: 'app-rent',
  standalone: true,
  imports: [DivisaPipe, ReactiveFormsModule, RouterModule, CalendarComponent],
...
export class RentComponent implements OnDestroy {
 ...
 enviar(){
  this.bookingService.saveBooking(this.vehicle!._id, this.form.value.fechaInicio,
    this.form.value.fechaFin, this.numDias * this.vehicle!.pricePerDay, this.form.value.codigoPromocional).subscribe({
      next: ()=>{
        Swal.fire({
